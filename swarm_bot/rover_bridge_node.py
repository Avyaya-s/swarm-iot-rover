...
class RoverMappingBridge(Node):
    def __init__(self):
        ...
        # 🛠️ NATIVE HARDWARE FALLBACK STATE VARIABLES
        ...
        # Broadcast the fixed physical attachment positions of sensors on the robot chassis
        self.publish_static_sensor_transforms()
        
        # 🔄 Deterministic execution loop (Runs at 20Hz regardless of network packet arrivals)
        self.timer = self.create_timer(0.05, self.bridge_tick)
        self.get_logger().info("🚀 Telemetry Mapping Bridge has initialized on Port 5000 (Simulation-Safe Mode).")

    def publish_static_sensor_transforms(self):
        """Defines exactly where each sensor is physically mounted relative to the robot's center (base_link)"""
        transforms = []

        # 1. Front Sensor: Mounted 10cm forward facing straight ahead (0 degrees)
        ...
        # 2. Left Sensor: Mounted 5cm forward, 5cm left, facing 90 degrees left
        ...
        # 3. Right Sensor: Mounted 5cm forward, 5cm right, facing 90 degrees right
    ...
    def bridge_tick(self):
        """Unified system cycle: Drains network packets, then updates metrics tracking state machine"""
        # Step A: Attempt to drain all pending UDP queue buffers to update states
        try:
            while True:
            ...
            # Buffer clear/No hardware transmitting yet. Transition to standard loop execution.
            pass
        except Exception as e:
            self.get_logger().error(f"Error parsing packet: {e}")

        # Step B: ALWAYS execute publishing streams (Simulates continuous runtime stability)
        current_time = self.get_clock().now()
        
        # 1. Broadcast dynamic position tracking transforms
        self.update_and_publish_odom(self.current_yaw, current_time, self.current_front_cm)
        
        # 2. Broadcast matching ultrasound distance maps
    ...
    def update_and_publish_odom(self, yaw_deg, current_time, front_cm):
        ...
        # --- SMART SPEED EMULATION BASED ON ARDUINO NAV MATRIX ---
